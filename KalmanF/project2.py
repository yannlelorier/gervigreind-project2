...
def set_lat_lon_from_x_y(flight):
    ...
    flight.data["longitude_after"] = lons
    flight.data["latitude_after"] = lats
    return flight
...
if __name__ == "__main__":
    ...
    set_lat_lon_from_x_y(radar_data[0])

    # lines_filt = plt.scatter(x = res[0], y = res[1], color='r')
    lines_filt = plt.scatter(x = radar_data[0].data[['latitude_after']].values, y= radar_data[0].data[['longitude_after']].values, color='r')
    plt.show()
